"""Averages observed phosphate levels by season and subcatchment."""
...
# 1. LOAD DATA
# -----------------------------
file_path = "Data_PP/Output/Final_With_Dates_Snapped.csv"
...
def assign_season(date):
    """Assign a season label based on the month of the date."""
    month = date.month
    year = date.year
    if month in [9, 10, 11]:
...
for season, season_df in df.groupby("Season"):
    # Remove 0 / NaN values
    season_df = season_df[
        season_df["Phosphate (Hanna)"].notna() & (season_df["Phosphate (Hanna)"] > 0)
    ]
...
# -----------------------------
output_dir = "Modelling/Phosphate_averages"
